...
for SNR = 0:LIMIT_SNR
    ...
    sinal_filtrado_NRZ = filter(filtro_rx, 1, sinal_rx_NRZ) / N;
    sinal_filtrado_RZ = filter(filtro_rx, 1, sinal_rx_RZ) / N;
    Z_T = sinal_filtrado_NRZ(N:N:end);
    info_hat_NRZ = Z_T > limiarNRZ;
    Z1_T = sinal_filtrado_RZ(N:N:end);
...
subplot(212)
plot(t, sinal_filtrado_NRZ)
xlim([0 10 * Tb])
ylim([-(2*A) 2*A])
hold on
plot(t(N:N:end), Z_T, 'o')
xlim([0 10 * Tb])
ylim([-(2*A) 2*A])
...
subplot(212)
plot(t, sinal_filtrado_RZ)
xlim([0 10 * Tb])
ylim([-(2*A) 2*A])
hold on
plot(t(N:N:end), Z1_T, 'o')
xlim([0 10 * Tb])
ylim([-(2*A) 2*A])
title('Sinal Filtrado')
